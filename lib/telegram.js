...
export async function sendTelegramNotification(type, data) {
  ...
  // 4. PENENTUAN JUDUL, PATH HALAMAN & PERUBAHAN TEKS NOTIFIKASI
  const BASE_URL = 'https://harvest-system-v2.vercel.app';
  let categoryName = 'Event Baru';
  let targetPath = '/events';

  if (categoryKey === 'contest') {
    categoryName = 'Contest Baru';
    targetPath = '/contest';
  } else if (categoryKey === 'training') {
    categoryName = 'Training Baru';
    targetPath = '/events';
  } else if (categoryKey === 'video') {
    categoryName = 'Video/Modul Pembelajaran Baru';
    targetPath = '/academy';
  } else if (categoryKey === 'production') {
    categoryName = 'Laporan Production Report';
  ...
  // Format Teks Caption Sesuai Permintaan
  let caption = `📢 <b>Admin Telah Menambahkan ${categoryName}!</b>\n\n`;
  caption += `📌 <b>Judul:</b> ${judulContent}\n\n`;
  caption += `👉 Klik link berikut untuk info lebih lanjut:\n`;
