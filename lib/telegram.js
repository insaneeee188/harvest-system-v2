...
export async function sendTelegramNotification(type, data) {
  ...
  // 4. PENENTUAN JUDUL, PATH HALAMAN & CAPTION KHUSUS
  ...
  if (categoryKey === 'contest') {
    ...
    targetPath = '/academy'; // Diperbarui dari /library ke /academy untuk mencegah 404
  ...
  const videoOrFileLink = data.link || data.youtubeUrl || data.url || fullLink;

  // Format Caption Pesan Telegram Baru
  let caption = `${categoryName} Telah Ditambahkan!\n`;
  caption += `Judul: ${judulContent}\n`;
  caption += `Link Video: ${escapeHtml(videoOrFileLink)}\n`;

  if (data.kategori) {
    caption += `Kategori: ${escapeHtml(data.kategori)}\n`;
  }

  caption += `\nKlik link berikut untuk info lebih lanjut:${BASE_URL}/`;
