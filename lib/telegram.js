...
export async function sendTelegramNotification(type, data) {
  ...
  // Format Caption Dinamis Sesuai Parameter dari Admin Panel / Webhook
  let caption = `📢 <b>${categoryName} Telah Ditambahkan!</b>\n\n`;
  caption += `📌 <b>Judul:</b> ${judulContent}\n`;

  // Penambahan Link YouTube dari Webhook jika tersedia
  if (data.link || data.youtubeUrl || data.url) {
    caption += `🔗 <b>Link Video:</b> ${escapeHtml(data.link || data.youtubeUrl || data.url)}\n`;
  }

  if (data.status) {
    caption += `🌐 <b>Status:</b> ${escapeHtml(data.status)}\n`;
  }
